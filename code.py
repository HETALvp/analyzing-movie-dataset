...
def duplicate_and_unique_movies(dataset, i):

    duplicate = []
    final = []
    # """Check the duplicate and unique entries.
    v = [movie[i] for movie in dataset]
    for movie in v:
...
def movies_lang(dataset, index_, lang_):
    ...
    #movies_ = list[filter(lambda x: True if x[index_] == lang_ else False,dataset)]
    v = [movie[index_] for movie in dataset]
    #print(v)
    for m in range(0,len(v)):
        if(v[m] == lang_):
            movies_.append(dataset[m])
    # Keyword arguments:
...
def rate_bucket(dataset, rate_low, rate_high):
    ...
    # Once you ahve extracted the movies, call the explore_data() to print first few rows.
    v = [movie[11] for movie in dataset]
    #print(v)
    for m in range(0,len(v)):
        if(float(v[m]) >= rate_low and float(v[m]) <= rate_high):
            rated_movies.append(dataset[m])
    # Keyword arguments:
    # dataset -- list containing the details of the movie
...
reviews_max = {}
reviews = [movie[12] for movie in movies]
movie_name = [movie[13] for movie in movies]
for m in range(0,len(reviews)):
    if(int(reviews[m]) >= 1):
        reviews_max.update({movie_name[m]:reviews[m]})
...
for key in reviews_max:
    if(key not in movies_clean):
        f.append(reviews_max.get(key))
    else:
        movies_clean.append(reviews_max.get(key))
...
# Call the rate_bucket function to see the
# movies with rating higher than 8.
